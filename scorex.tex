...
\begin{enumerate}
	...
	\item Transaction throughput: As mentioned in the previous point, the maximum block size is 1 MB. This roughly translates to about an average of 144 MB of data added to the blockchain per day if all blocks are full. Assuming a transaction size of the about 224 bytes (the minimum), we get a maximum of 28086 transactions/hour, far less than centralized payment systems such as VISA.
\end{enumerate}
...
In this method, a node only verifies headers from the genesis block and validates only the last few full blocks (enough for rollback). The key idea in SPV is that the security of PoWs is not measured by block height (the number of blocks since the genesis block) but rather the depth (the number of blocks mined since a particular payment has been confirmed). This allows clients be able to verify payments made to them. 

\subsection{Rollerchain}

Rollerchain (RC) is a proposal aimed to (1) reduce storage and (2) enforces nodes to store blocks and prevent tragedy of the commons. We will look at each separately. Firstly, assume that nodes are altruistic (i.e., they will store data to help others as long as the data is not too large). RC stores data only for verifing recent $k$ iterations for some security parameter $k$. This is similar to SPV mode. However, the difference is that instead of each node storing the blocks (the deltas of minimal states), the nodes store the minimal state itself. The block header contains a merkle root hash of the minimal state at the start of that iteration along with the merkle root hash of the transactions in that block. To address the second problem, a node is required to store a subset of the last $k$ minimal states obtained by hashing its public key. When mining, a node must provide PoW and additionally, a proof that the required states are indeed stored. [...]
...
ZeroCash (ZC) is a blockchain based protocol designed for privacy. Since the data in the Bitcoin blockchain is public, true anonymity is not present. ZC can be considered an extension of ZeroCoin (ZCn), an earlier proposal. In order to describe ZC, we will first describe ZCn.

Assume that all coins to be obfuscated are of a single denomination, say 1 BTC. ZCn has its own currency called Z, such that 1 Z = 1 BTC, there is a pool of obfuscated Z coins called ``zeropool''. Alice wants to anonymize her bitcoin represented by serial number $c_A$. In order to do that, she begins by exchanging $c_A$ with an equivalent Z coin as follows. She generates a secret serial number $s_A$, a random $r_A$ and creates a commitment to $s_A$ as $Z_A=$ COMM$(r_A,s_A)$. This is the coin she uses in exchange for $c_A$. All such exchanged coins are automatically added to the zeropool. The commitment has a computationally binding and perfectly hiding property so no knowledge of $s_A$ or $r_A$ is leaked from $Z_A$. Later on Alice will take back her $Z_A$ using a transaction $t_A$, which is then automatically converted to 1 BTC. For anonymity, an adversary should not be able to link $t_A$ to $Z_A$. This is achieved using Non-interactive Zero-Knowledge Proofs (NIZKs).\footnote{A NIZK proof is essentially understood as follows. Let $N$ be an instance of a hard problem, such as finding the factors of a large integer. Let $p$ be a solution to this problem that Alice knows. There are three requirements of NIZK proofs: (1) Given a \textbf{random} string $r$, Alice can construct a short proof $\pi=$ PROOF$(p, r)$ such that VERIFY$(\pi, r, N)=true$ iff Alice `really knows p'. The concept of `knowing' is captured as follows: (2) If Alice can be fooled into using a \textbf{maliciously crafted $r$}, then $\pi$ will reveal $p$ completely. (3) Additionally, Alice can construct pairs $(\pi', r')$ such that VERIFY$(\pi', r', N)$ is true without knowing $p$ as long as \textbf{she selects $r'$}. Thus, the generation of $r$ is of outmost importance because if the verifier selects $r$, the secret is leaked and if the prover selects $r$ then the proof is not convincing. If given the transcript of a proof, we cannot distinguish between the cases (1) and (3), then the protocol is zero knowledge, intuitively because because given some $(\pi, r)$ pair, there is no way of knowing if it was generated using (3) or (1).} 

Alice creates a spend by revealing $s_A$ (but not $r_A$) and creates a NIZK proof of the statement ``I know $r$ such that $(r, s_A)$ form a valid commitment from the given list of commitments $L= C_1, C_2, \ldots C_n$'', where $L$ contains $C_A$. The process of creating a commitment and revealing only one input (but never opening them completely) may seem counterintuitive, but this in fact makes the statement provable by efficient NIZKs, because the circuit for computing the commitment can be efficiently coded into an NP language. The spend could have been done even without revealing $s_A$. However, this is needed for the next property -- security -- so that Alice should not be able to spend $Z_A$ twice. This is achieved by keeping track of spent $s_A$s in a public ledger, kept by each client. This has two scalability issues: (1) The zero-pool always grows as we cannot determine which of the Z coins have been spent (for anonymity). The spent set always grows as we need to keep track of double spends. Unlike Bitcoin, which uses spends only for bootstrapping and stores only unspent outputs, ZCn must store both the spent and unspent output. The unspent outputs is simply the zero-pool. 

ZC is an extension of ZC with the following differences:
\begin{enumerate}
	...
	\item ZC allows coins of arbitrary denominations instead of 1 BTC as in ZCn. 
	\item The statement  ``I know $r$ such that $(r, s_A)$ form a valid commitment from the given list of commitments $L= C_1, C_2, \ldots C_n$'' is replaced by the statement  ``I know $r$ such that $(r, s_A)$ form a root node of the Merkle tree of commitments with root hash $M$''. This makes the statement much shorter.
	\item ZC uses shorter NIZKs called zkSNARKS (succinct non-interactive arguments of knowledge). A zkSNARK is essentially a NIZK with the proof sizes and verification-time O(1).
\end{enumerate}
