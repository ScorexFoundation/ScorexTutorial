...
The goal of a fullnode node is to get the same \textit{minimal state}~(see Section~\ref{min-state} for details) representation eventually for a version $h$ as other honest nodes, and also obtain some specific information for the node users from the history and the state. To accomplish the goal, as described in the section~\ref{fullnodeview}, the node has a local view of the world comprising of four entities:
