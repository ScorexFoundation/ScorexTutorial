...
\section{Transaction} Roughly speaking, a transaction consists of a set of \emph{inputs} (source of funds) and \emph{outputs} (destination of funds).
In a simplest case, funds are exchanged between {\em addresses} which are hashes of public keys. We will sometimes use the terms `address' and `public key' interchangeably. However, it should be remembered that addresses are one-way hashes of public keys. %Depending on the context, a symbol $A$ could refer to a public key or an address (the meaning will be clear from the usage).
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item  {\em KeyGen$(k)$:} This takes as input a security parameter $k$ and outputs a (public-key, private-key) pair $(pk, sk)$. 
		\item  {\em Sign$(sk, m)$:} This takes as input a private-key $sk$ and a message $m$. It outputs a signature $s$.
		\item  {\em Verify$(pk, m, s)$:} This takes as input a public-key $pk$, a message $m$ and a signature $s$. It outputs True or False.
	\end{enumerate}
	Correctness requires that for all $(pk, sk)$ pairs output from {\em KeyGen} and for all $m$, it must hold that {\em Verify}$(pk, m, ${\em Sign}$(sk, m)) = $ True.
	...
	Security requires that this is the only way to generate valid signature. Formally, we define the security using a game where we give the attacker access to the signing oracle for some private key $sk$ and require him to generate a valid signature (one that verifies with $pk$) on a message that was never queried to this oracle.
	...
	\item {\em Commitments:} Some proposals (such as ZeroCash~\cite{sasson2014zerocash}) use commitments. A commitment scheme allows a user, say Alice, to commit to a message $m$ without revealing  it. Instead she sends a commitment $c =$ COMM$_r(m)$, where $r$ is a random value. Later on, she can open her commitment by revealing $(r, m)$. A verifier then computes VERIFY$(c, m, r )$ and outputs either True or False. For correctness, we require that for all $(r, m)$, it must hold that VERIFY$($COMM$_r(m), m, r) =$ True.
...
Randomizes consensus in open networks with unknown number of participants is pretty new and also a very hard question. 
...
%\knote{In Bitcoin, we hash the whole header. Some words about nonce field are needed}
...
The key idea in SPV is that the security is not measured by a block's height (the number of blocks before it since the genesis block) but rather its depth (the number of blocks mined after it) \knote{???}. However, the security of SPV has not been formally proven.
