...
The newer applications of blockchain are all based on how these rules can be written and how expressive the language is. For instance, Etherium, another blockchain based protocol boasts of a Turing complete language in contrast to the tiny one that Bitcoin provides. The following summarizes the main proposals.
...
\subsubsection{Transaction:} Roughly speaking, a transaction consists of a set of \emph{inputs} (source of funds) and \emph{outputs} (destination of funds). %The funds available at the inputs are pooled together and are available for transfer to the outputs. 
...
%\label{sec:verify}
%Every transaction sent from the client eventually reaches every other client with a high probability. 
A new transaction is valid if all the references are unused. %the inputs from which the funds are being sent refer to unspent outputs. 
%Each client maintains a list of unspent outputs and rejects the transaction if the referenced output is not in this list. Another reason to reject a transaction would be if the sum of funds at input is less than that at outputs, or if some of the signatures do not verify. If all checks pass, 
