...
\anote{Not clear what the below sentence means.}

In Scorex we assume there is a dedicated reactive component holding a view node with a direct modification of it from outside prohibited.

A \textit{node view holder} is a component that manages and updates this view when it receives either an offchain object (such as an offchain transaction), or an on-chain object~(such as a block (header), a key-block/micro-block in Bitcoin-NG, a blockheader/fullblock/state-snapshot in RollerChain). We call such a object a \textit{persistent node view modifier} having a signature of the following type:
...
\begin{enumerate}
	\item The first is an \textit{offchain transaction}, which is not part of any history log, and will be processed solely using a memory pool and a wallet. 
...
A concrete implementation of a modifier must provide: (1) An type-identifer for the type of modifier, (2) An identifier for the modifier itself, and (3) A method giving a binary representation of the identifier:
...
Using our model, we define methods to validate a transaction: 
