...
Function \textit{compare} is checking a SyncInfo instance got from another party and returns one of \textit{HistoryComparisonResult} values provided below: 
...
Function \textit{openSurfaceIds} returns last block if a history is linear, otherwise it returns last blocks from a blocktree etc. That is why it is returning sequence of modifier identifiers~(Seq[ModifierId]). 
