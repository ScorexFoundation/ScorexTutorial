...
What do you need to do in order to build something on Scorex is to provide implementations for all the abstract
...
The goal of a fullnode node is to get the same \textit{minimal state}~(see Section~\ref{min-state} for details) representation eventually for a version $h$ as other honest nodes, and also some specific information for the node users. To accomplish the goal, as described in the section~\ref{fullnodeview}, the node has a local view of the world comprising of four entities:
...
This component, a \textit{node view holder}, modifies a node view it holds on getting whether an offchain object, so an offchain transaction, or an on-chain object~(a block, or key-block/micro-block in Bitcoin-NG, a blockheader/fullblock/state-snapshot in RollerChain etc.). Signatures of a trait and its sub-components are as follows:
...
A \textit{vault} is a storage for node-specific information
